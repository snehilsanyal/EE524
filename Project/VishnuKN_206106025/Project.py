...
predictioncalc=[]
newpredcal=[]
ntrials=len(traineegfeatures)
for i in range(0,len(traineegfeatures)):
    distancepoint=[]
    comparedata=testrecord[i]

    print('--------------------------------------------')
    print(i)
    distancepoint=other_euclid(testrecord,comparedata)


    sortednew,newind=bubbleSort(distancepoint)
    #newind=newind[newind!=]
    #sortednew=sortednew[sortednew!=0]
    newneighbors.append(newind[0:(k+1)])
    newlabel.append(labels[newind[0:(k+1)]])
    newpredcal.append(stats.mode(newlabel[i]))
    newpred.append(newpredcal[i][0][0])
    print('Distance Square')
    print('Neighbors -- > ')
    print(newneighbors[i])
    print('Prediction New --> %d' %newpred[i])
    #print('Label' ---> newlabel[i])
    print('-----------------------------------------------')
correctlabel=labels
